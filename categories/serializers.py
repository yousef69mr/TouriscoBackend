...
class TicketClassCategorySerializer(serializers.ModelSerializer):
...
class TicketClassCategoriesSerializer(serializers.ModelSerializer):
    category = TourismCategorySerializer(source='categoryObject',read_only=True)
    language = LanguageSerializer(source='lang',read_only=True)
    ...
    def create(self, validated_data):
        ...
        instance = TicketClassCategoryLanguageBased.objects.create(categoryObject=category, lang=language, **validated_data)
        # print(instance)
        translatedInstance = translate_django_model(instance, instance.lang.code.lower())
